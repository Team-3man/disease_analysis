...
- 양희준 : 코로나 확진 여부/백신 접종 여부, 건강검진 여부에 따른 심혈관 및 폐 질환, 건강행태의 연관성 탐구
...
## 🧭 기대 효과
✅ 1) 질병 위험요인에 대한 직관적 이해 제공

생활습관(BMI, 운동, 음주, 수면)과 주요 질환(심혈관, 폐질환, 우울증 등)의 관계를
시각적으로 직접 확인함으로써
어떤 요인이 어떤 질병과 더 강하게 연결되는지 한눈에 파악할 수 있다.

→ “어떤 건강습관이 가장 위험한가?”를 데이터로 명확히 제시

⸻

✅ 2) 고위험군을 조기에 파악하는 데 도움

데이터 기반으로 위험지수를 만들고 비교함으로써
심혈관 위험군, 폐질환 취약군, 정신건강 취약군 등을 빠르게 식별할 수 있다.

→ 의료·보건기관이 선제적 개입 대상자를 선정하는 데 활용 가능
→ 기업·학교·지역사회 건강관리 프로그램에도 적용 가능

⸻

✅ 3) 정책적인 의사결정에 활용 가능

심혈관·폐질환·정신건강에 영향을 주는
“변화 가능한(modifiable) 위험요인”이 무엇인지 파악하면
	•	운동 프로그램 확대
	•	흡연/음주 정책 보완
	•	수면 교육 캠페인
	•	비만 예방 정책

등 실제 정책 강화 방향을 제안할 수 있다.

⸻

✅ 4) 생활습관 중심의 예방의학 강화

심혈관 질환의 80%가 ‘수정 가능한 요인’ 때문이므로
본 분석은 개인 단위 예방의학 전략(예: 운동, 체중 관리, 수면 개선)을
데이터로 뒷받침할 수 있다.

→ “운동하면 실제로 어떻게 달라질까?”를 그래프로 설명 가능
→ 환자 교육 자료로도 활용 가능

⸻

✅ 5) 팬데믹 이후 건강행태 변화를 이해하는 데 기여

코로나 확진 여부, 백신 접종 여부, 의료 검진 경험(HIV 검사 등)을 함께 비교함으로써,
	•	감염 이후 어떤 건강 문제가 증가했는지
	•	어떤 그룹이 더 취약한지
	•	건강검진 접근성이 건강 상태와 어떻게 연결되는지

등 포스트 코로나 시대의 건강 트렌드를 분석하는 데 도움이 된다.
...
코로나·백신(감염병),
우울증(정신건강)
등 매우 다양한 분야를 하나의 데이터로 분석했다.

→ 향후 머신러닝 모델이나 인과관계 분석 등 고도 분석으로 확장 가능
→ 의료 AI 개발에도 응용 가능
...
BMI, 흡연, 수면, 운동 같은 기본 생활습관이 모든 질병군에서 공통적으로 가장 큰 영향을 주는 요인으로 확인되었다.
심혈관질환은 고령·비만·흡연·운동 부족에서 위험이 크게 증가했고, 우울증은 여성 / 수면 및 정신건강 지표와 밀접하게 연결되었다.
폐질환 역시 코로나 여부보다 흡연, 비만, 운동 여부가 더 강한 차이를 만들었다.
전반적으로 특정 질병의 종류를 가리지 않고, 생활습관 패턴의 질이 건강위험을 결정하는 핵심 변수로 작용함
따라서 생활습관을 관리하는 것만으로도 상당 부분 질병 위험을 예방하거나 완화할 가능성이 있음을 시사한다.
...
| 양희준 | 이번 EDA 프로젝트로 데이터 전처리부터 시각화까지 전체 흐름을 이해하는데 도움이 되었습니다. <br> 처음에 코로나 확진 여부나 백신 접종 여부에 따라 건강 지표가 뚜렷하게 나올 것이라고 예상했는데, <br> 실제 데이터 다뤄보니 달라서 기본 작업을 좀 더 세밀하게 해야겠다 생각 들었습니다.  |
| 김승주 | 데이터 자체가 관계성이 있는 데이터들이 아니라서 약간 아쉬웠습니다. <br> 거의 다 범주형데이터여서 다양한 그래프를 써보지 못한것도 아쉬웠습니다. <br> 만약 다음 기회가 있다면 금융 돤련 데이터 해보고 싶어요. |
